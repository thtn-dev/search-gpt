...
async def register(
    *, session: AsyncSession = Depends(get_async_session), register_dto: UserCreate
):
    """
...
async def login(
    *,
    ...
    access_token = create_access_token(
        subject=str(db_user.id), additional_claims=additional_claims
    )
...
async def handle_nextauth_signin(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                'User Get/Create successful & committed. User system ID: %d',
                db_user_model.id,
            )
            ...
            logger.info(
                'Found user after commit failed, likely race condition resolved.'
            )
        ...
        # Unexpected errors
        logger.error(
            'Rolling back due to unexpected error in NextAuth signin endpoint: ',
            exc_info=True,
        )
...
async def refresh_token(body: RefreshTokenRequest = Body(...)):
    """
