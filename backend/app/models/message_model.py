...
class ContentMetadataType(types.TypeDecorator):
    ...
    def process_bind_param(
        self, value: Optional[ContentMetadata], dialect
...
class MessageBase(SQLModel):
    ...
    msg_metadata: ContentMetadata = Field(
        default=None, sa_column=Column(ContentMetadataType, nullable=True, index=False)
    )

    format: str = Field()

    content: str = Field(sa_column=Column(Text, nullable=False, index=False))
